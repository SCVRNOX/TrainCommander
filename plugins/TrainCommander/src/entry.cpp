...
static void NexusLog(ELogLevel aLevel, const char *aChannel,
                     const char *aStr) {
  if (APIDefs && APIDefs->Log) {
    APIDefs->Log(aLevel, aChannel, aStr);
  }
}

static std::string GetAddonDirectory(AddonAPI_t *api,
                                     const char *addonName) {
  if (!api)
    return std::string();

  if (api->Paths_GetAddonDirectory)
    return api->Paths_GetAddonDirectory(addonName);

  if (api->Paths_GetGameDirectory) {
    std::string root = api->Paths_GetGameDirectory();
    if (!root.empty()) {
      if (root.back() == '\\' || root.back() == '/')
        return root + "addons\\" + addonName;
      return root + "\\addons\\" + addonName;
    }
  }
...
void AddonLoad(AddonAPI_t *aApi) {
  ...
  std::string addonDir = GetAddonDirectory(APIDefs, "TrainCommander");
  if (addonDir.empty()) {
    NexusLog(LOGL_WARNING, "TrainCommander",
             "Paths_GetAddonDirectory unavailable; using fallback path.");
    addonDir = "TrainCommander";
  }

  g_Manager = new TrainManager(addonDir);
  g_Manager->LoadTrains();

  g_Catalog = new EventCatalog(addonDir, APIDefs);
  ...
  if (APIDefs->InputBinds_RegisterWithString) {
    ...
    NexusLog(LOGL_WARNING, "TrainCommander",
             "Input binds unavailable in this Nexus API version.");
  }
  ...
  // Register Renderer
  if (APIDefs->GUI_Register) {
    APIDefs->GUI_Register(RT_Render, AddonRender);
    APIDefs->GUI_Register(RT_OptionsRender, AddonOptions);
  } else {
    NexusLog(LOGL_WARNING, "TrainCommander",
             "GUI_Register unavailable in this Nexus API version.");
  }

  NexusLog(LOGL_INFO, "TrainCommander", "Addon loaded successfully.");
}

// Cleanup on unload
void AddonUnload() {
  if (APIDefs) {
    if (APIDefs->GUI_Deregister) {
  ...
  NexusLog(LOGL_INFO, "TrainCommander", "Addon unloaded.");
}
